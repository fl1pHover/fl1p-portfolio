...
const TechCard: React.FC<TechCardProps> = ({ src, alt, text }) => {
  return (
    <Link href="/" className="bg-black aspect-6/5 duration-75 group flex flex-col items-center justify-center p-4 hover:bg-white/10 hover:border-x inset-0 border-border relative">
      {/* <img src={src} alt={alt} className="w-3/4 h-auto object-contain" /> */}
      ...
      <span className="text-xs uppercase bottom-2 left-2 absolute opacity-0 group-hover:opacity-100 duration-75">
       [ Next.js ]
      </span>
    </Link>
  );
